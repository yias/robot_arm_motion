...
Eigen::VectorXf _currentTargetPos(3); // current target position
Eigen::VectorXf _nextTargetPos(3); // next target position
...
int main(int argc, char** argv)
{
    ...
    // define subscriber to listen to robot ee pose and if norm is <= x, then update target by multiplying [1,-1,1]
    ...
    geometry_msgs::Pose target_msg1;
    ...
    if (parse_target_params("ram_target_position", "ram_target_orientation", &target_marker, &target_msg1) < 0) {
        std::cout << "[obstacle-target simulation] Correct the obstacles parameters in yaml file and launch again the node" << std::endl;
        return -2;
    }
    ...
    geometry_msgs::Pose target_msg2;

    target_msg2.position.x = -0.5;
    target_msg2.position.y = -0.5;
    target_msg2.position.z = 0.12;

    target_msg2.orientation.w = 0;
    target_msg2.orientation.x = 0;
    target_msg2.orientation.y = 1;
    target_msg2.orientation.z = 0;

    _currentTargetPos << target_msg1.position.x, target_msg1.position.y, target_msg1.position.z;

    ros::Rate loop_rate(100);

    while (ros::ok()) {

        std::cout << "Distance from target: " << (_eePosition - _currentTargetPos).norm() << "\n";

        // update target
        if ((_eePosition - _currentTargetPos).norm() <= 0.5 && switchTarget == false) {
            _pubTarget.publish(target_msg2);
            _currentTargetPos << target_msg2.position.x, target_msg2.position.y, target_msg2.position.z;
            switchTarget = true;
        }

        if ((_eePosition - _currentTargetPos).norm() <= 0.5 && switchTarget) {
            _pubTarget.publish(target_msg1);
            _currentTargetPos << target_msg1.position.x, target_msg1.position.y, target_msg1.position.z;
            switchTarget = false;
        }
        // std::cout << "(_eePosition - _targetPosition).norm(): " << (_eePosition - _currentTargetPos).norm() << "\n";
        std::cout << "target_msg1.position.y: " << target_msg1.position.y << "\n";
        std::cout << "target_msg2.position.y: " << target_msg2.position.y << "\n";
        // std::cout << "target_msg.position.y: " << target_msg.position.y << "\n";
